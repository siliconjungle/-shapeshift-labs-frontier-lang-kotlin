# @shapeshift-labs/frontier-lang-kotlin

Kotlin PSI source-language importer package for Frontier Lang semantic source documents.

Wraps the compiler Kotlin PSI native importer with package-level metadata, import helpers, and semantic sidecar generation for Kotlin PSI/KtFile-shaped syntax trees.
...
```js
import { importKotlinSource, createKotlinSemanticImportSidecar } from '@shapeshift-labs/frontier-lang-kotlin';
...
const imported = await importKotlinSource({
  ...
  ast: {
    kind: 'KtFile',
    packageDirective: { kind: 'KtPackageDirective', fqName: 'demo' },
    declarations: [{
      kind: 'KtClass',
      name: 'Todo',
      declarations: [{
        kind: 'KtPrimaryConstructor',
        parameters: [{ kind: 'KtParameter', name: 'title', typeReference: { text: 'String' }, valOrVarKeyword: 'val' }]
      }, {
...
const sidecar = await createKotlinSemanticImportSidecar({ sourcePath: 'src/Todo.kt', ast: imported.nativeAst });
console.log(imported.metadata.astFormat);
console.log(sidecar.symbols.length);
```

## API

- `KotlinLanguagePackage`
- `createKotlinNativeImporterAdapter(options)`
- `createKotlinPsiNativeImporterAdapter(options)`
- `importKotlinSource(input, options)`
- `createKotlinSemanticImportSidecar(input, options)`

This package does not bundle Kotlin compiler or IntelliJ dependencies. Callers own parsing and may inject `ast`, `nativeAst`, `ktFile`, `file`, `sourceFile`, `root`, or a parser module. Kotlin Analysis API, FIR/K2, compiler-plugin, KSP/KAPT, multiplatform, script, and build-variant semantics must be attached as evidence if a merge flow wants to claim those semantics.
